...
namespace RiveScript
{
	...
	public class RiveScript
    {
		...
		private IDictionary<string, string> _objects;

		// name->language mappers
		[MessagePackMember(2)]
		public IDictionary<string, ObjectMacro> objects = new Dictionary<string, ObjectMacro>();

        // Simpler internal data structures.
		[MessagePackMember(3)]
        public IDictionary<string, string> globals = new Dictionary<string, string>();                             // ! global

		[MessagePackMember(4)]
        public IDictionary<string, string> vars = new Dictionary<string, string>();                                // ! var

		[MessagePackMember(6)]
        public IDictionary<string, ICollection<string>> arrays = new Dictionary<string, ICollection<string>>();    // ! array

		[MessagePackMember(7)]
        public IDictionary<string, string> subs = new Dictionary<string, string>();                                // ! sub
		[MessagePackMember(8)]
        public string[] subs_s = null;                                                                             // sorted subs
		[MessagePackMember(9)]
        public IDictionary<string, string> person = new Dictionary<string, string>();                              // ! person
		[MessagePackMember(10)]
        public string[] person_s = null; // sorted persons

        private string _currentUser = Constants.Undefined;

		// Regexes don't serialize correctly.
		[MessagePackMember(11)]
		public IDictionary<string, string> RegexSerialized {
			get {
				return RegexCache.ToDictionary (kvp => kvp.Key, kvp => kvp.Value.ToString ());
			}
			set {
				RegexCache = value.ToDictionary (kvp => kvp.Key, kvp => new Regex (kvp.Value)); 
			}
		}

		private IDictionary<string, Regex> RegexCache = new Dictionary<string, Regex> ();

//		public MsgPack.MessagePackObjectDictionary RegexCache = new MsgPack.MessagePackObjectDictionary ();
        /// <summary>
        /// Create a new RiveScript interpreter object, specifying the debug mode.
        /// </summary>
        /// <param name="debug">Is true Enable debug mode (ai*lot* of stuff is printed to the terminal)</param>
        ...
        /// <returns></returns>
		public bool stream(string code, bool replace)
        {
            // Split the given code up into lines.
            var lines = code.Split(new[] { "\n" }, StringSplitOptions.None);

            // Send the lines to the parser.
            return parse("(streamed)", lines, replace);
        }
		...
		public bool stream(string[] code, bool replace)
        {
			...
			return parse("(streamed)", code, replace);
        }
        ...
        /// <param name="handler">An insiance of a class that implements an ObjectHandler.</param>
        public void setHandler(string name, IObjectHandler handler)
        {
			handlers.Add(name, handler);
			foreach(string macroName in this.objects.Keys) {
				if (name == objects[macroName].Lang) {
					handler.onLoad (macroName, objects [macroName].Code);
				}
			}
		}
        ...
        /// <returns></returns>
        public bool setUservars(string user, Dictionary<string, string> data)
        {
		...
		private bool parse(string filename, string[] code, bool replace)
        {
            ...
            Trigger onTrig = null;    // Trigger we're on
                ...
                if (inobj) {
                    if (line.StartsWith("<object") || line.StartsWith("< object")) { 
                      ...
                      if (handlers.ContainsKey(objLang)) {
						...
						if (replace && objects.ContainsKey (objName)) {
							objects [objName] = new ObjectMacro (objLang, objBuff.ToArray ());
						} else {
							objects.Add(objName, new ObjectMacro(objLang, objBuff.ToArray()));
						}
                      }
                      objName = "";
                      objLang = "";
...
//						onTrig = topics.topic(topic).trigger(line, replace);
						onTrig = topics.topic(topic).trigger(line + "{previous}" + isThat, replace);
						onTrig.hasPrevious(true);
...
//					Trigger t = topics.topic(topic).trigger(onTrig);
					onTrig.addReply(line);
                }
        ...
        private string reply(string user, string message, bool begin, int step)
        {
                ...
                for (int i = 0; i < allTopics.Length; i++)
                {
                    ...
                    if (this.topics.topic(allTopics[i]).hasPrevious())
                    {
                        ...
                        for (int j = 0; j < previous.Length; j++)
                        {
                            ...
                            // Does it match?
							Regex re = triggerRegexp(user, profile, previous[j]); //new Regex("^" + regexp + "$");
                            foreach (Match m in re.Matches(lastReply))
                            {
                                ...
                                for (int k = 0; k < candidates.Length; k++)
                                {
                                    ...
                                    Regex reH = triggerRegexp(user, profile, candidates[k]);
                                    //say("Compare " + message + " <=> " + candidates[k] + " (" + humanside + ")");

									//Regex reH = humanside// new Regex("^" + humanside + "$");
            ...
            if (foundMatch == false)
            {
                ...
                for (int a = 0; a < triggers.Length; a++)
                {
                    ...
                    Regex re= triggerRegexp(user, profile, trigger);
        ...
        /// <returns></returns>
        private Regex triggerRegexp(string user, Client profile, string trigger)
        {
			if (RegexCache.ContainsKey (trigger))
				return RegexCache[trigger];
			...
			RegexCache [trigger] = new Regex ("^" + regexp + "$");
			return RegexCache[trigger];
        }
		...
		public void printReplies(string trigger) {
			foreach (string topic in this.topics.listTopics()) {
				if (this.topics.topic (topic).triggers.ContainsKey (trigger)) {
					foreach (string r in this.topics.topic (topic).triggers [trigger].listReplies ()) {
						Debug.Log (r);
					}
				}
			}
        ...
        {
            if (debug)
				Debug.LogError("<RS> " + text + " at " + file + " line " + line + ".");
            else
