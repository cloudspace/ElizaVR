...
{
    /// <summary>
    /// Topic class for RiveScript
    /// </summary>
	[Serializable]
    public class Topic
    {
        private static bool debug = false;
        public Dictionary<string, Trigger> triggers = new Dictionary<string, Trigger>(); // Topics contain triggers
        public bool _hasPrevious = false; // Has at least one %Previous
        public Dictionary<string, ICollection<string>> previous = new Dictionary<string, ICollection<string>>();// Mapping of %Previous's to their triggers
        public ICollection<string> _includes = new List<string>();// Included topics
        public ICollection<string> _inherits = new List<string>();// Inherited topics
        public string[] sorted = null; // Sorted trigger list
        ...
        /// <returns></returns>
		public Trigger trigger(string pattern, bool replace)
        {
            if (false == triggers.ContainsKey(pattern)) {
                triggers.Add(pattern, new Trigger(name, pattern));
            }
			if (replace) {
				triggers[pattern] = new Trigger(name, pattern);
			}
