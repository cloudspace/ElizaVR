...
namespace RiveScript.lang
{
    public class CSharp : IObjectHandler
    {
        ...
        public bool onLoad(string name, string[] code)
        {
			...
			// TODO: This may be taking some time when running this csharp code
			// Cloudspace.VoiceProcessor.main.GetChatbot().dumpSorted();
            var ass = CreateAssembly(name, code);
			var method = ass.GetType(ns + "." + name).GetMethod("method");
			var del = (Func<RiveScript, string[], string>)System.Delegate.CreateDelegate(typeof(Func<RiveScript, string[], string>), method);
